...
docker run --privileged --network=dmz --ip 192.1.2.4 -p20:20 -p21:21 -tdi --name=ftpserver stilliard/pure-ftpd bash
...
##############################################################################################################################
docker run  --privileged --sysctl net.ipv4.ip_forward=1 --network=rete_esterna --ip 192.1.1.5 -td --name=firewall1 alfonso991999/firewallimg bash
...
docker exec --privileged -t firewall1 iptables -t filter -A FORWARD -i eth0 -o eth1 -m state --state NEW,ESTABLISHED,RELATED -j DROP
...
# Inoltro tutto il resto dei pacchetti provenienti dall'interno (eth0) sull'interfaccia della DMZ (eth1)	                     
